...
5. Before committing, run `npm run typecheck` and `npm run test`
...
7. Two-axis review against the base specified in your task: `git diff {base}..HEAD`
   ...
   **Standards axis** — does the diff conform to this repo's coding standards?
   - Read `AGENTS.md` and check every hunk against its rules (type safety, code conventions, terminology, etc.)
   ...
   **Spec axis** — does the diff faithfully implement the issue?
   ...
   If you find issues in either axis, fix them, re-run `npm run typecheck && npm run test`, then commit with `Review #42:` prefix. If the review found nothing to fix, note that in your output.

<if-model is="deepseek/*">
DeepSeek is strong at spotting structural complexity. During the Standards axis, focus extra attention on: unnecessary abstraction layers, deeply nested conditionals, and functions that could be simplified without changing behavior.
<else>
<if-model is="anthropic/*">
Claude benefits from explicit review prompts. After your first review pass, do a second pass specifically looking for: missing edge cases, type-safety gaps, and places where a comment would prevent future confusion.
</if-model>
</if-model>
...
<if-model is="deepseek/*, openai-codex/*">
These models are cost-effective for implementation work. Spend your token budget on thorough TDD coverage and clean refactoring — be precise but not verbose.
</if-model>
