...
3. **Tests and typecheck pass** on your working branch before you start — workers will fail if the baseline is already broken:
   ```bash
...
- Runs `pnpm typecheck && pnpm test` before committing
...
Fix the baseline first on your working branch. Workers run `pnpm typecheck && pnpm test` before committing — if those are already broken they'll abort and report it.
